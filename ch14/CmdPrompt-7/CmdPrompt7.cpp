/*
프로그램 설명 : 명령 프롬프트 프로젝트 7 (CmdPrompt7.cpp)
project 1 : exit
project 2 : calc, notepad 등 외부 프로그램 실행 (CreateProcess)
...
BOOL CopyDirectoryFiles(WIN32_FIND_DATA fileData, TCHAR* source, TCHAR* dest)
{
	...
	if (!_tcscmp(fileData.cFileName, _T(".")) || !_tcscmp(fileData.cFileName, _T(".."))) {
		// .는 현재 디렉터리, ..는 상위 디렉터리 의미하는 문자. 아무런 처리하지 않고 돌아감
	}
	// directory일 때 처리 (하위 directory 생성하고 XCOPY() 함수 call)
		...
		XCOPY(subSourceDir, subDestDir); // 재귀 방법
	}
	// file 일 때는 file copy 수행
	else {
